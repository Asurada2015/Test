...
def R2_of_batch(logits, targets):
    ...
    logits_Yeild = tf.unstack(tf.cast(trans_logits, tf.double))
    trans_targets = tf.transpose(targets)
    targets_Yeild = tf.unstack(tf.cast(trans_targets, tf.double))
...
for i in tqdm.tqdm(range(generations)):
    ...
    if i >= 100:
        if (i + 1)%output_every == 0:
            train_R2_Y = sess.run(R2_of_batch(model_output, train_targets), {is_training: True})
            train_RMSE_Y = sess.run(RMSE_of_batch(model_output, train_targets),
                                    {is_training: True})
            ...
            log_train[1].append(train_R2_Y)
            log_train[2].append(train_RMSE_Y)
            print(output)  # 显示训练集上的loss值

        # 显示在测试集上各项指标

        if (i + 1)%eval_every == 0:
            test_R2_Y = sess.run(R2_of_batch(test_output, test_targets), {is_training: False})
            test_RMSE_Y = sess.run(RMSE_of_batch(test_output, test_targets),
                                   {is_training: False})
            test_loss_value = sess.run(loss_in_testdata, {is_training: False})
            test_RPD_Y = sess.run(RPD_of_batch(test_output, test_targets), {is_training: False})
            ...
            log_test[1].append(test_R2_Y)
            log_test[2].append(test_RMSE_Y)
            log_test[3].append(test_RPD_Y)
