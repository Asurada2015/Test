...
def read_data(file_queue):
    ...
    vertor_label = tf.stack([Elongation])
...
def R2_of_batch(logits, targets):
    ...
    logits_Elongation = tf.unstack(tf.cast(trans_logits, tf.double))
    trans_targets = tf.transpose(targets)
    targets_Elongation = tf.unstack(tf.cast(trans_targets, tf.double))
    r2_Elongation = R2(logits_Elongation, targets_Elongation)
    return r2_Elongation
...
for i in tqdm.tqdm(range(generations)):
    ...
    if i >= 100:
        if (i + 1)%output_every == 0:
            train_R2_E = sess.run(R2_of_batch(model_output, train_targets), {is_training: True})
            train_RMSE_E = sess.run(RMSE_of_batch(model_output, train_targets),
                                    {is_training: True})
            ...
            log_train[1].append(train_R2_E)
            log_train[2].append(train_RMSE_E)
            print(output)  # 显示训练集上的loss值

        # 显示在测试集上各项指标

        if (i + 1)%eval_every == 0:
            test_R2_E = sess.run(R2_of_batch(test_output, test_targets), {is_training: False})
            test_RMSE_E = sess.run(RMSE_of_batch(test_output, test_targets),
                                   {is_training: False})
            test_loss_value = sess.run(loss_in_testdata, {is_training: False})
            test_RPD_E = sess.run(RPD_of_batch(test_output, test_targets), {is_training: False})
