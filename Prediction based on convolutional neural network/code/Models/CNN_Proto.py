...
output_every = 2  # 训练输出间隔
generations = 100  # 迭代次数 20000
eval_every = 1  # 测试输出间隔
...
with tf.variable_scope('model_definition') as scope:
    ...
    # print(sess.run(tf.shape(model_output)))  # [128,3]
    # trans_model_output = tf.transpose(model_output)

    """"""
    # print(sess.run(tf.shape(trans_model_output)))  # [3,128]
    # Tensile, Yeild, Elongation = tf.unstack(trans_model_output)
    """"""

    # print(sess.run(tf.shape(Tensile))) [128]
    # print(sess.run(tf.shape(Yeild))) [128]
    # print(sess.run(tf.shape(Elongation))) [128]
# 声明损失函数
