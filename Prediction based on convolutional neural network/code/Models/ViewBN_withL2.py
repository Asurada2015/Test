...
REGULARAZTION_RATE = 0.0001  # 正则化项在损失函数中的系数,如果使用0值则表示不使用正则项
Dropout_rate = 0.1  # 神经元选择性失活概率(丢包率)
...
def inference(input_images, batch_size, is_training):
    ...
    with tf.variable_scope('full1') as scope:
        ...
        full_layer1 = tf.layers.dropout(full_layer1, rate=0.1, training=is_training)
